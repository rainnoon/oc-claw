...
function DailyChart({ stats }: { stats: DailyStats[] }) {
  ...
  const unitLabel = isTokens ? (scale === 1_000_000 ? 'M tokens' : scale === 1_000 ? 'K tokens' : 'tokens') : '消息数'
        ...
        <div className="flex">
          ...
          <div className="flex-1 flex items-end gap-px" style={{ height: chartH, borderLeft: '1px solid rgba(255,255,255,0.1)', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingLeft: 1 }}>
            {stats.map((d, i) => {
              ...
              const tip = isTokens ? `${d.date}: ${formatTokens(v)}` : `${d.date}: ${v} 条`
              return (
    ...
    <div className="px-5 py-5 flex flex-col gap-6 overflow-y-auto scrollbar-thin" style={{ maxHeight: 524 }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          <h1 className="text-lg font-semibold text-white tracking-tight">Claude Code 统计</h1>
          {stats.model && <span className="text-xs text-white/40">{stats.model}</span>}
        </div>
        <span className="text-xs text-white/40">近 14 天</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex flex-col gap-1 hover:bg-white/[0.05] transition-colors">
          <span className="text-[10px] font-medium text-white/40 uppercase tracking-wider">总 Token</span>
          <span className="text-2xl font-semibold text-white tracking-tight mt-1">{formatTokens(totalTokens)}</span>
          <span className="text-xs text-white/40">{stats.totalSessions} 个会话</span>
        </div>
        <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex flex-col gap-1 hover:bg-white/[0.05] transition-colors">
          <span className="text-[10px] font-medium text-white/40 uppercase tracking-wider">消息数</span>
          <span className="text-2xl font-semibold text-white tracking-tight mt-1">{stats.totalMessages}</span>
          <span className="text-xs text-white/40">AI 回复</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 bg-white/[0.03] border border-white/5 rounded-2xl p-4 hover:bg-white/[0.05] transition-colors">
        <span className="text-[10px] font-medium text-white/40 uppercase tracking-wider">Token 明细</span>
