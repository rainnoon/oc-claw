...
2. **Hook script**: `~/.Codex/hooks/ooclaw-codex-hook.sh` (Unix) / `.ps1` (Windows), regenerated by `install_claude_hooks()` via `install_codex_hooks()`.
3. **Supported events installed by oc-claw**: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PermissionRequest`, `Stop`, `StopFailure`, `SubagentStop`.
...
- The correct approach: write a `.ps1` file and register the command as `powershell.exe -NoProfile -ExecutionPolicy Bypass -File 'C:/Users/xxx/.../ooclaw-*-hook.ps1'` (forward slashes).
